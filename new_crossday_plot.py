...
def get_channelmap(ChannelmapPath,CMname,RecordingSystem,FPC):
    ...
    ## intan channel
    ch_map = ChannelMapDf.sort_values(by=column_name[0])[RF].to_numpy()
...
def calculate_valid_channel(crossday_recording):
...
## 对于没有最新日期的，test_timestamp输入[]
    ...
    for animal in Probe_list:
        try:
            ...
            if len(test_timestamp)& is_update_all:
              ## 对于已经有最新日期的，判断数据库最新和飞书表格最新是否一致      
                latest_day = crossday_recording['Date_string'][-1]
                
                ind = Probe_list.index(animal)
                    
                test_time2= datetime.fromtimestamp(test_timestamp[ind] / 1000.0)

                test_time2.replace(tzinfo=tz_utc_8)
                latest_day = datetime.strptime(latest_day,'%y%m%d')
                ## 如果一致则跳过
                if (latest_day-test_time2).days==0:
                        print("已是最新日期")
                        continue
            ...
            ini_func,each_func = calculate_valid_channel(crossday_recording)
...
update_plot(Probe_list2,test_timestamp2)
